...
class FMenuBuilder;
class FAdvancePakEditorLogTextLayoutMarshaller;
class FTextLayout;
class SMenuAnchor;
...
*/
struct FAdvancePakEditorLogMessage
{
	...
	FAdvancePakEditorLogMessage(const TSharedRef<FString>& NewMessage, FName NewCategory, FName NewStyle = NAME_None)
		: Message(NewMessage)
	...
	FAdvancePakEditorLogMessage(const TSharedRef<FString>& NewMessage, ELogVerbosity::Type NewVerbosity, FName NewCategory, FName NewStyle = NAME_None)
		: Message(NewMessage)
...
*/
struct FAdvancePakEditorLogFilter
{
	/** true to show Logs. */
	bool bShowLogs;
	...
	FAdvancePakEditorLogFilter() : TextFilterExpressionEvaluator(ETextFilterExpressionEvaluatorMode::BasicString)
	{
	...
	bool IsMessageAllowed(const TSharedPtr<FAdvancePakEditorLogMessage>& Message);
 ...
 */
class SAdvancePakEditorLog 
	: public SCompoundWidget, public FOutputDevice
{

public:

	SLATE_BEGIN_ARGS( SAdvancePakEditorLog )
		: _Messages()
		{}
		
		/** All messages captured before this log window has been created */
		SLATE_ARGUMENT( TArray< TSharedPtr<FAdvancePakEditorLogMessage> >, Messages )

	SLATE_END_ARGS()

	/** Futuretructor for output log, so we can unregister from notifications */
	virtual ~SAdvancePakEditorLog();
	...
	/**
	 * Creates FAdvancePakEditorLogMessage objects from FOutputDevice log callback
	 *
	 * @param	V Message text
	 * @param Verbosity Message verbosity
	 * @param Category Message category
	 * @param OutMessages Array to receive created FAdvancePakEditorLogMessage messages
	 * @param Filters [Optional] Filters to apply to Messages
	 *
	 * @return true if any messages have been created, false otherwise
	 */
	static bool CreateLogMessages(const TCHAR* V, ELogVerbosity::Type Verbosity, const class FName& Category, TArray< TSharedPtr<FAdvancePakEditorLogMessage> >& OutMessages);
...
protected:
	...
	/** Absorbs the array of messages into something the text box understands */
	TSharedPtr< FAdvancePakEditorLogTextLayoutMarshaller > MessagesTextMarshaller;
...
public:
	...
	FAdvancePakEditorLogFilter Filter;

	TSharedPtr<class SComboButton> ViewOptionsComboButton;
};

/** Output log text marshaller to Absorb an array of FAdvancePakEditorLogMessages into styled lines to be consumed by an FTextLayout */
class FAdvancePakEditorLogTextLayoutMarshaller : public FBaseTextLayoutMarshaller
{
public:

	static TSharedRef< FAdvancePakEditorLogTextLayoutMarshaller > Create(TArray< TSharedPtr<FAdvancePakEditorLogMessage> > InMessages, FAdvancePakEditorLogFilter* InFilter);

	virtual ~FAdvancePakEditorLogTextLayoutMarshaller();
...
protected:
	...
	FAdvancePakEditorLogTextLayoutMarshaller(TArray< TSharedPtr<FAdvancePakEditorLogMessage> > InMessages, FAdvancePakEditorLogFilter* InFilter);
