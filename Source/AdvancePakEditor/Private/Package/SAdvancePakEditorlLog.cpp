...
#include "SAdvancePakEditorLog.h"
...
{
public:
	explicit FLogFilter_TextFilterExpressionContext(const FAdvancePakEditorLogMessage& InMessage) : Message(&InMessage) {}
...
private:
	...
	const FAdvancePakEditorLogMessage* Message;
};
...
void SConsoleInputBox::Construct(const FArguments& InArgs)
{
	...
	if (!ConsoleCommandCustomExec.IsBound()) // custom execs always show the default executor in the UI (which has the selector disabled)
	{
		...
		if (GConfig->GetString(TEXT("AdvancePakEditorLog"), TEXT("PreferredCommandExecutor"), PreferredCommandExecutorStr, GEditorPerProjectIni))
		{
...
//	FSlateApplication::Get().PromotePathToWidgetUnchecked( InputText.ToSharedRef(), WidgetToFocusPath );
...
{
	GConfig->SetString(TEXT("AdvancePakEditorLog"), TEXT("PreferredCommandExecutor"), *InExecName.ToString(), GEditorPerProjectIni);
...
TSharedRef< FAdvancePakEditorLogTextLayoutMarshaller > FAdvancePakEditorLogTextLayoutMarshaller::Create(TArray< TSharedPtr<FAdvancePakEditorLogMessage> > InMessages, FAdvancePakEditorLogFilter* InFilter)
{
	return MakeShareable(new FAdvancePakEditorLogTextLayoutMarshaller(MoveTemp(InMessages), InFilter));
}

FAdvancePakEditorLogTextLayoutMarshaller::~FAdvancePakEditorLogTextLayoutMarshaller()
{
}

void FAdvancePakEditorLogTextLayoutMarshaller::SetText(const FString& SourceString, FTextLayout& TargetTextLayout)
{
...
void FAdvancePakEditorLogTextLayoutMarshaller::GetText(FString& TargetString, const FTextLayout& SourceTextLayout)
{
	SourceTextLayout.GetAsText(TargetString);
}

bool FAdvancePakEditorLogTextLayoutMarshaller::AppendPendingMessage(const TCHAR* InText, const ELogVerbosity::Type InVerbosity, const FName& InCategory)
{
	return SAdvancePakEditorLog::CreateLogMessages(InText, InVerbosity, InCategory, Messages);
}

bool FAdvancePakEditorLogTextLayoutMarshaller::SubmitPendingMessages()
{
...
void FAdvancePakEditorLogTextLayoutMarshaller::AppendPendingMessagesToTextLayout()
{
	const int32 CurrentMessagesCount = Messages.Num();
	...
	{
		const TSharedPtr<FAdvancePakEditorLogMessage> Message = Messages[MessageIndex];
...
void FAdvancePakEditorLogTextLayoutMarshaller::ClearMessages()
{
	NextPendingMessageIndex = 0;
	Messages.Empty();
	MakeDirty();
...
void FAdvancePakEditorLogTextLayoutMarshaller::CountMessages()
{
	...
	{
		const TSharedPtr<FAdvancePakEditorLogMessage> CurrentMessage = Messages[MessageIndex];
...
int32 FAdvancePakEditorLogTextLayoutMarshaller::GetNumMessages() const
{
	const int32 NumPendingMessages = Messages.Num() - NextPendingMessageIndex;
	return Messages.Num() - NumPendingMessages;
}

int32 FAdvancePakEditorLogTextLayoutMarshaller::GetNumFilteredMessages()
{
...
void FAdvancePakEditorLogTextLayoutMarshaller::MarkMessagesCacheAsDirty()
{
	bNumMessagesCacheDirty = true;
}

FAdvancePakEditorLogTextLayoutMarshaller::FAdvancePakEditorLogTextLayoutMarshaller(TArray< TSharedPtr<FAdvancePakEditorLogMessage> > InMessages, FAdvancePakEditorLogFilter* InFilter)
...
BEGIN_SLATE_FUNCTION_BUILD_OPTIMIZATION
void SAdvancePakEditorLog::Construct( const FArguments& InArgs )
{
	...
	Filter.ToggleLogCategory(FName("AdvancePakLog"));
	...
	MessagesTextBox = SNew(SMultiLineEditableTextBox)
		...
		.AutoWrapText(this, &SAdvancePakEditorLog::IsWordWrapEnabled)
		.OnVScrollBarUserScrolled(this, &SAdvancePakEditorLog::OnUserScrolled)
		.ContextMenuExtender(this, &SAdvancePakEditorLog::ExtendTextBoxMenu);
		...
		[
			...
			[
				...
				[
					...
					.OnGetMenuContent(this, &SAdvancePakEditorLog::MakeAddFilterMenu)
					.HasDownArrow(true)
				...
				[
					...
					.OnTextChanged(this, &SAdvancePakEditorLog::OnFilterTextChanged)
					.OnTextCommitted(this, &SAdvancePakEditorLog::OnFilterTextCommitted)
			...
			[
				...
				[
					...
					[
						...
						.OnConsoleCommandExecuted(this, &SAdvancePakEditorLog::OnConsoleCommandExecuted)
	...
	FCoreDelegates::OnHandleSystemError.AddRaw(this, &SAdvancePakEditorLog::OnCrash);
...
SAdvancePakEditorLog::~SAdvancePakEditorLog()
{
	if (GLog != nullptr)
	{
...
void SAdvancePakEditorLog::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
...
void SAdvancePakEditorLog::OnCrash()
{
	if (GLog != nullptr)
	{
		GLog->RemoveOutputDevice(this);
...
bool SAdvancePakEditorLog::CreateLogMessages( const TCHAR* V, ELogVerbosity::Type Verbosity, const class FName& Category, TArray< TSharedPtr<FAdvancePakEditorLogMessage> >& OutMessages )
{
			...
			if (!LineRange.IsEmpty())
			{
				...
				for (int32 CurrentStartIndex = 0; CurrentStartIndex < Line.Len();)
				{
					...
					if (bIsFirstLineInMessage)
					{
						...
						OutMessages.Add(MakeShared<FAdvancePakEditorLogMessage>(MakeShared<FString>(MessagePrefix + HardWrapLine), Verbosity, Category, Style));
					}
					else
					{
						...
						OutMessages.Add(MakeShared<FAdvancePakEditorLogMessage>(MakeShared<FString>(MoveTemp(HardWrapLine)), Verbosity, Category, Style));
					}
...
void SAdvancePakEditorLog::Serialize(const TCHAR* V, ELogVerbosity::Type Verbosity, const class FName& Category)
{
	MessagesTextMarshaller->AppendPendingMessage(V, Verbosity, Category);
}

FSlateColor SAdvancePakEditorLog::GetViewButtonForegroundColor() const
{
...
{
	FUIAction ClearAdvancePakEditorLogAction(
		FExecuteAction::CreateRaw( this, &SAdvancePakEditorLog::OnClearLog ),
		FCanExecuteAction::CreateSP( this, &SAdvancePakEditorLog::CanClearLog )
		);

	Builder.AddMenuEntry(
		NSLOCTEXT("AdvancePakEditorLog", "ClearLogLabel", "Clear Log"), 
		NSLOCTEXT("AdvancePakEditorLog", "ClearLogTooltip", "Clears all log messages"), 
		FSlateIcon(), 
		ClearAdvancePakEditorLogAction
		);
}

void SAdvancePakEditorLog::OnClearLog()
{
...
void SAdvancePakEditorLog::OnUserScrolled(float ScrollOffset)
{
	bIsUserScrolled = ScrollOffset < 1.0 && !FMath::IsNearlyEqual(ScrollOffset, 1.0f);
}

bool SAdvancePakEditorLog::CanClearLog() const
{
	return MessagesTextMarshaller->GetNumMessages() > 0;
}

void SAdvancePakEditorLog::OnConsoleCommandExecuted()
{
...
void SAdvancePakEditorLog::RequestForceScroll()
{
...
void SAdvancePakEditorLog::Renovate()
{
	// Re-count messages if filter changed before we refresh
...
bool SAdvancePakEditorLog::IsWordWrapEnabled() const
{
	bool WordWrapEnabled = false;
	GConfig->GetBool(TEXT("/Script/UnrealEd.EditorPerProjectUserConfigures"), TEXT("bEnableAdvancePakEditorLogWordWrap"), WordWrapEnabled, GEditorPerProjectIni);
	return WordWrapEnabled;
}

void SAdvancePakEditorLog::SetWordWrapEnabled(ECheckBoxState InValue)
{
	const bool WordWrapEnabled = (InValue == ECheckBoxState::Checked);
	GConfig->SetBool(TEXT("/Script/UnrealEd.EditorPerProjectUserConfigures"), TEXT("bEnableAdvancePakEditorLogWordWrap"), WordWrapEnabled, GEditorPerProjectIni);
...
bool SAdvancePakEditorLog::IsClearOnPIEEnabled() const
{
	bool ClearOnPIEEnabled = false;
	GConfig->GetBool(TEXT("/Script/UnrealEd.EditorPerProjectUserConfigures"), TEXT("bEnableAdvancePakEditorLogClearOnPIE"), ClearOnPIEEnabled, GEditorPerProjectIni);
	return ClearOnPIEEnabled;
}

void SAdvancePakEditorLog::SetClearOnPIE(ECheckBoxState InValue)
{
	const bool ClearOnPIEEnabled = (InValue == ECheckBoxState::Checked);
	GConfig->SetBool(TEXT("/Script/UnrealEd.EditorPerProjectUserConfigures"), TEXT("bEnableAdvancePakEditorLogClearOnPIE"), ClearOnPIEEnabled, GEditorPerProjectIni);
}

void SAdvancePakEditorLog::OnFilterTextChanged(const FText& InFilterText)
{
...
void SAdvancePakEditorLog::OnFilterTextCommitted(const FText& InFilterText, ETextCommit::Type InCommitType)
{
	OnFilterTextChanged(InFilterText);
}

TSharedRef<SWidget> SAdvancePakEditorLog::MakeAddFilterMenu()
{
	FMenuBuilder MenuBuilder(/*bInShouldCloseWindowAfterMenuSelection=*/true, nullptr);
	
	MenuBuilder.BeginSection("AdvancePakEditorLogVerbosityEntries", LOCTEXT("AdvancePakEditorLogVerbosityHeading", "Verbosity"));
	{
		MenuBuilder.AddMenuEntry(
			LOCTEXT("ShowMessages", "Messages"), 
			LOCTEXT("ShowMessages_Tooltip", "Filter the Output Log to show messages"), 
			FSlateIcon(), 
			FUIAction(FExecuteAction::CreateSP(this, &SAdvancePakEditorLog::VerbosityLogs_Execute), 
				FCanExecuteAction::CreateLambda([] { return true; }), 
				FIsActionChecked::CreateSP(this, &SAdvancePakEditorLog::VerbosityLogs_IsChecked)), 
			NAME_None, 
		...
		MenuBuilder.AddMenuEntry(
			...
			FSlateIcon(), 
			FUIAction(FExecuteAction::CreateSP(this, &SAdvancePakEditorLog::VerbosityWarnings_Execute), 
				FCanExecuteAction::CreateLambda([] { return true; }), 
				FIsActionChecked::CreateSP(this, &SAdvancePakEditorLog::VerbosityWarnings_IsChecked)), 
			NAME_None, 
			EUserInterfaceActionType::ToggleButton
		);
		...
		MenuBuilder.AddMenuEntry(
			...
			FSlateIcon(), 
			FUIAction(FExecuteAction::CreateSP(this, &SAdvancePakEditorLog::VerbosityErrors_Execute), 
				FCanExecuteAction::CreateLambda([] { return true; }), 
				FIsActionChecked::CreateSP(this, &SAdvancePakEditorLog::VerbosityErrors_IsChecked)), 
			NAME_None, 
			EUserInterfaceActionType::ToggleButton
		);
	}
	...
	MenuBuilder.BeginSection("AdvancePakEditorLogMiscEntries", LOCTEXT("AdvancePakEditorLogMiscHeading", "Miscellaneous"));
	{
		MenuBuilder.AddSubMenu(
			LOCTEXT("Categories", "Categories"), 
			LOCTEXT("SelectCategoriesToolTip", "Select Categories to display."), 
			FNewMenuDelegate::CreateSP(this, &SAdvancePakEditorLog::MakeSelectCategoriesSubMenu)
		);
	}
...
void SAdvancePakEditorLog::MakeSelectCategoriesSubMenu(FMenuBuilder& MenuBuilder)
{
	MenuBuilder.BeginSection("AdvancePakEditorLogCategoriesEntries");
	{
		MenuBuilder.AddMenuEntry(
			LOCTEXT("ShowAllCategories", "Show All"),
			LOCTEXT("ShowAllCategories_Tooltip", "Filter the Output Log to show all categories"),
			FSlateIcon(),
			FUIAction(FExecuteAction::CreateSP(this, &SAdvancePakEditorLog::CategoriesShowAll_Execute),
			FCanExecuteAction::CreateLambda([] { return true; }),
			FIsActionChecked::CreateSP(this, &SAdvancePakEditorLog::CategoriesShowAll_IsChecked)),
			NAME_None,
			EUserInterfaceActionType::ToggleButton
		);
		...
		for (const FName Category : Filter.GetAvailableLogCategories())
		{
			MenuBuilder.AddMenuEntry(
				...
				FSlateIcon(),
				FUIAction(FExecuteAction::CreateSP(this, &SAdvancePakEditorLog::CategoriesSingle_Execute, Category),
				FCanExecuteAction::CreateLambda([] { return true; }),
				FIsActionChecked::CreateSP(this, &SAdvancePakEditorLog::CategoriesSingle_IsChecked, Category)),
				NAME_None,
				EUserInterfaceActionType::ToggleButton
			);
		}
...
bool SAdvancePakEditorLog::VerbosityLogs_IsChecked() const
{
	return Filter.bShowLogs;
}

bool SAdvancePakEditorLog::VerbosityWarnings_IsChecked() const
{
	return Filter.bShowWarnings;
}

bool SAdvancePakEditorLog::VerbosityErrors_IsChecked() const
{
	return Filter.bShowErrors;
}

void SAdvancePakEditorLog::VerbosityLogs_Execute()
{ 
...
void SAdvancePakEditorLog::VerbosityWarnings_Execute()
{
...
void SAdvancePakEditorLog::VerbosityErrors_Execute()
{
...
bool SAdvancePakEditorLog::CategoriesShowAll_IsChecked() const
{
	return Filter.bShowAllCategories;
}

bool SAdvancePakEditorLog::CategoriesSingle_IsChecked(FName InName) const
{
	return Filter.IsLogCategoryEnabled(InName);
}

void SAdvancePakEditorLog::CategoriesShowAll_Execute()
{
...
void SAdvancePakEditorLog::CategoriesSingle_Execute(FName InName)
{
...
TSharedRef<SWidget> SAdvancePakEditorLog::GetViewButtonContent()
{
	...
	MenuBuilder.AddMenuEntry(
		...
		FUIAction(
			...
			FCanExecuteAction::CreateLambda([] { return true; }),
			FIsActionChecked::CreateSP(this, &SAdvancePakEditorLog::IsWordWrapEnabled)
		),
		NAME_None,
		EUserInterfaceActionType::ToggleButton
	);
	MenuBuilder.AddMenuEntry(
		...
		FUIAction(
			...
			FCanExecuteAction::CreateLambda([] { return true; }),
			FIsActionChecked::CreateSP(this, &SAdvancePakEditorLog::IsClearOnPIEEnabled)
		),
		NAME_None,
		EUserInterfaceActionType::ToggleButton
	);
...
void SAdvancePakEditorLog::OpenLogFileInExplorer()
{
...
void SAdvancePakEditorLog::OpenLogFileInExternalEditor()
{
...
bool FAdvancePakEditorLogFilter::IsMessageAllowed(const TSharedPtr<FAdvancePakEditorLogMessage>& Message)
{
	// Filter Verbosity
	{
...
void FAdvancePakEditorLogFilter::AddAvailableLogCategory(FName& LogCategory)
{
...
void FAdvancePakEditorLogFilter::ToggleLogCategory(const FName& LogCategory)
{
...
bool FAdvancePakEditorLogFilter::IsLogCategoryEnabled(const FName& LogCategory) const
{
	return SelectedLogCategories.Contains(LogCategory);
}

void FAdvancePakEditorLogFilter::ClearSelectedLogCategories()
{
